...
public class LevelCraftCore extends JavaPlugin {
	...
	public boolean Reload() {
		...
		for (Plugin plugin : pm.getPlugins()) {
			if (plugin.getDescription().getName().startsWith("LC")) {
				...
				String[] str =			(String[]) getConfig().getStringList("ReferenceKeys").toArray();
				String[] exp =			(String[]) getConfig().getStringList("LevelExpPer").toArray();
				String[] unlocks =		(String[]) getConfig().getStringList("LevelUnlocks").toArray();
				String[] help =			(String[]) getConfig().getStringList("LevelHelp").toArray();
				String index =			getConfig().getString("ReferenceIndex");
				String name =			getConfig().getString("LevelName");
				String author =			getConfig().getString("Author");
				int[] unlockslevel = {};
				try {
					unlockslevel = intArr(getConfig().getIntegerList("LevelUnlocksLevel"));
				} catch (Exception e) {
					e.printStackTrace();
				}
	...
	public void loadConfig() {
		...
		this.database = getConfig().getString("Database", "FlatFile");
		this.MySqlDir = getConfig().getString("MySqlDatabaseDirectory",
				"localhost:3306/minecraft");
		this.MySqlUser = getConfig().getString("MySqlDatabaseUsername", "root");
		this.MySqlPass = getConfig().getString("MySqlDatabasePassword", "");
		this.EnableLevelCap = getConfig().getBoolean("EnableLevelCap", true);
		this.LevelCap = getConfig().getInt("LevelCap", 100);
		this.Constant = getConfig().getInt("LevelConstant", 20);
		this.EnableSkillMastery = getConfig().getBoolean("EnableSkillMastery", true);
		this.c1 = getConfig().getString("ColourOne", "GOLD");
		this.c2 = getConfig().getString("ColourTwo", "YELLOW");
		this.c3 = getConfig().getString("ColourGood", "GREEN");
		this.c4 = getConfig().getString("ColourBad", "RED");
		this.UnlockLines = getConfig().getInt("UnlockLines", 7);
		this.ExpLines = getConfig().getInt("ExpLines", 7);
		this.NotifyAll = getConfig().getBoolean("NotifyAll", true);
		String byRaw = getConfig().getString("LevelBypassers", "Dave,Rick,Player1337");
		this.Bypassers = byRaw.split(",");
		this.anticheat = getConfig().getBoolean("AntiBoost", true);
		this.LevelUpSound = getConfig()
				.getString("LevelUpSound",
						"http://cloud.github.com/downloads/samkio/Levelcraft/FFLevelUp.wav");
		this.EnableSound = getConfig().getBoolean("EnableSound", true);
		this.EnableCapes = getConfig().getBoolean("EnableCape", true);
		this.PeriodicSave = getConfig().getBoolean("PeriodicSave", false);
		this.SaveInterval = getConfig().getInt("SaveInterval", 300);
		// this.Specialisation = getConfig().getBoolean("Specialisation", false);
		// this.SpecialMultpilier = getConfig().getDouble("SpecialMultiplier", 2);
		List<World> worldRun = this.getServer().getWorlds();
		String str = "";
		for (World w : worldRun) {
			str = str + w.getName() + ",";
		}
		String worldsraw = getConfig().getString("Worlds", str);
	...
	public void registerEvents() {
		getServer().getPluginManager().registerEvents(new LCPlayerListener(this), this);
	}
	...
	public int[] intArr(List<Integer> intlist) throws Exception {
		if (intlist != null) {
